...
object Migrations {
    ...
    const val CURRENT_VERSION: Int = 52
    ...
    @JvmField
    val MIGRATION_25_31 = object : Migration(26, 31) {
        override fun migrate(database: SupportSQLiteDatabase) {
    ...
    @JvmField
    val MIGRATION_38_43 = object : Migration(38, 43) {
        override fun migrate(database: SupportSQLiteDatabase) {

            database.execSQL("CREATE TABLE down(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title TEXT, length INTEGER NOT NULL," +
                    " path TEXT, link TEXT, chapter_id INTEGER NOT NULL, state TEXT" +
                    ", chap_number TEXT, manga_id INTEGER NOT NULL DEFAULT 0, FOREIGN KEY(chapter_id) REFERENCES chapter(id) ON UPDATE CASCADE ON DELETE CASCADE," +
                    " FOREIGN KEY(manga_id) REFERENCES manga(id) ON UPDATE CASCADE ON DELETE CASCADE)")

            database.execSQL("INSERT INTO down(id, title, length, path, link, chapter_id, state, chap_number)" +
                    " SELECT id, title, length, path, link, chapter_id, state, chap_number FROM download")
            ...
            database.execSQL("CREATE INDEX download_manga_id_index ON download(manga_id)")
        }
    }

    @JvmField
    val MIGRATION_43_52 = object : Migration(43, 52) {
        override fun migrate(database: SupportSQLiteDatabase) {

            database.execSQL("CREATE TABLE man (id INTEGER NOT NULL, title TEXT, " +
                    "link TEXT, alt_title TEXT, thumbnailLink TEXT, author TEXT, artist TEXT, summary TEXT, " +
                    " rating REAL NOT NULL, genres TEXT, votes INTEGER NOT NULL, views TEXT, " +
                    "popularity TEXT, type TEXT, status TEXT, _release INTEGER NOT NULL, " +
                    "bookmark INTEGER NOT NULL, PRIMARY KEY(id))")

            database.execSQL("INSERT INTO man(id, title, link, thumbnailLink, author, artist, " +
                    "summary, rating, genres, votes, views, popularity, type, status, _release, bookmark) SELECT * FROM manga")
            ...
            database.execSQL("CREATE TABLE chapters (id INTEGER NOT NULL PRIMARY KEY, number REAL NOT NULL DEFAULT 0, " +
                    "version TEXT, release_date INTEGER NOT NULL DEFAULT 0, link TEXT, bookmarked INTEGER NOT NULL," +
                    " completed	INTEGER NOT NULL, lastReadingPosition INTEGER NOT NULL, length INTEGER NOT NULL, title TEXT," +
                    " last_read_time INTEGER NOT NULL, new_chapter INTEGER NOT NULL, position INTEGER NOT NULL DEFAULT 0, manga_id INTEGER NOT NULL," +
                    " FOREIGN KEY(manga_id) REFERENCES manga(id) ON UPDATE CASCADE ON DELETE CASCADE)")

            database.execSQL("INSERT INTO chapters(id, version, bookmarked, completed," +
                    " lastReadingPosition, length, title, last_read_time, new_chapter, manga_id) " +
                    "SELECT id, version, bookmarked, completed, lastReadingPosition," +
                    " length, title, last_read_time, new_chapter, manga_id FROM chapter")
            ...
            database.execSQL("CREATE UNIQUE INDEX chapter_id_index ON chapter(link, version, manga_id, number)")
        }
    }
}
